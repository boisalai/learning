...
def get_default_directory() -> Path:
    """Retourne le répertoire de téléchargement par défaut selon l'OS"""
    if os.name == 'nt':  # Windows
        return Path(os.path.expanduser("~")) / "Downloads"
    elif os.name == 'posix':  # MacOS/Linux
        return Path(os.path.expanduser("~")) / "Downloads"
    else:
        return Path.cwd()  # Répertoire courant si OS non reconnu
...
class DocumentConfig:
    ...
    def create_report_style(cls, **kwargs):
        ...
        # Mettre à jour la configuration par défaut avec les kwargs
...
class MarkdownToDocx:
    ...
    def _post_process_document(self, doc_path: Path) -> None:
        ...
        if doc.paragraphs:
            ...
            title_para.clear()
            # Appliquer le style titre
            title_para.style = doc.styles['Title']
            # Créer un run avec le titre et son style
    ...
    def _apply_global_styles(self, doc: Document) -> None:
        ...
        # Configuration du style de titre
        try:
        ...
        # Configuration du style Normal pour tout le document
        ...
        # Configuration du style Default Paragraph Font
        try:
        ...
        # Configuration des styles de paragraphe standard
        ...
        # Configuration des sections et pieds de page
    ...
    def _process_paragraphs(self, doc: Document, doc_path: Path) -> None:
        ...
        for para in doc.paragraphs:
            ...
            # Application des styles selon le type de paragraphe
    ...
    def _process_tables(self, doc: Document) -> None:
        ...
        for table in doc.tables:
            ...
            for i, row in enumerate(table.rows):
                # Ignorer la première ligne car déjà traitée
                if i > 0:  
    ...
    def _process_table_cell(self, cell: _Cell, is_header: bool = False) -> None:
        ...
        Args:
            cell: La cellule à traiter
            is_header: True si la cellule fait partie de l'en-tête
        """
        for para in cell.paragraphs:
            # Ajuster l'espacement avant/après
            para.paragraph_format.space_before = Pt(2)
            para.paragraph_format.space_after = Pt(2)
            
            # Définir la police plus petite que le texte principal
            ...
            for run in para.runs:
                ...
                # Mettre en gras si c'est l'en-tête
                if is_header:
                    run.font.bold = True
        
            # Si le paragraphe est vide ou ne contient que des espaces
            if not para.text.strip():
                # Ajouter un run avec un espace pour maintenir la mise en forme
                run = para.add_run(" ")
                run.font.name = self.config.font_name
                run.font.size = Pt(table_font_size)
                if is_header:
    ...
    def convert(self, 
        ...
        cmd = [
            ...
            '--wrap=none',
            '--columns=999',
            '--resource-path', str(img_dir),  # Spécifier le chemin des ressources
...
def create_doc1():
    try:
        ...
        # Les images doivent être placées dans le sous-répertoire /img/
        converter.convert(
            input_file="input.md",
...
def create_doc2():
    try:
        # Création d'un rapport avec configuration personnalisée
        config = DocumentConfig.create_report_style(
            title="Mon Rapport",
            author="John Doe",
            date="2024-01-10",
            ...
            base_font_size=12,  
            heading_colors={
                1: (0, 0, 0),  # Titres niveau 1 en bleu (0, 0, 255)
                2: (0, 0, 0),  # Titres niveau 2 en vert (0, 128, 0)
                3: (0, 0, 0)   # Titres niveau 3 en rouge (128, 0, 0)
            },
            footer_text={
                "odd": "Mon Entreprise",
        ...
        # Création du convertisseur avec la configuration
