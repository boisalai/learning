...
class MarkdownToDocxConverter:
    ...
    def _apply_global_styles(self, doc: Document) -> None:
        ...
        for level in range(1, 4):  # Headings 1-3
            style_name = f'Heading {level}'
            color = self.config.heading_colors.get(level, (0, 0, 0))
            styles_to_configure[style_name] = {
                'font_size': 18 - (level * 2),  # H1=16, H2=14, H3=12
                'font_name': self.config.font_name,
                'bold': True,
                'space_before': 18,
                'space_after': 12,
    ...
    def _process_paragraphs(self, doc: Document) -> None:
        ...
        for para in doc.paragraphs:
            if not para.style or para.style.name == 'Normal':
                # Only apply direct formatting to paragraphs without style or with Normal style
